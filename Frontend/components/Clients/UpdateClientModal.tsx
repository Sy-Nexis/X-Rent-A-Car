...
export default function UpdateClientModal({ isOpen, onClose, client, onActionComplete }: UpdateClientModalProps) {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      // Endpoint mapping: http://localhost:5001/api/clients/update?government={id}
      const response = await fetch(`http://localhost:5001/api/clients/update?government=${client.government_id}`, {
        method: "PUT",
