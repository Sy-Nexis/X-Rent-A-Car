...
export default function FleetEmptyView({ onEnterVehicle }: FleetEmptyViewProps) {
  return (
    <div className="flex-1 overflow-y-auto p-8 custom-scrollbar relative flex flex-col justify-between">
      {/* Top Breadcrumbs */}
      <div>
        ...
        {/* Center Panel (span 3) */}
        <div className="bg-white rounded-2xl border border-gray-150 p-12 shadow-sm min-h-[440px] flex flex-col items-center justify-center text-center mb-8 relative overflow-hidden">
          {/* Subtle logo badge with warning dot */}
          <div className="relative mb-6">
            <div className="w-20 h-20 rounded-2xl bg-[#f1f5f9] flex items-center justify-center text-slate-700">
            ...
            <span className="absolute -bottom-1 -right-1 w-6 h-6 rounded-full bg-rose-50 border-4 border-white flex items-center justify-center text-[10px] text-rose-500 font-extrabold shadow-sm">
              ⚠
            </span>
          </div>

          <h2 className="text-2xl font-black text-slate-805 mb-3 tracking-tight">
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-center">
            ...
            <button className="flex items-center gap-2 border border-cyan-400 hover:bg-cyan-50/20 text-cyan-600 text-xs font-extrabold uppercase tracking-wider px-6 py-3.5 rounded-lg transition-all active:scale-[0.98]">
              Import CSV
            </button>
          </div>
        </div>
